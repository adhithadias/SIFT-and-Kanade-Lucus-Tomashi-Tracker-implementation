...
for i=1:size(D)(2)
  ...
  h = 1;
  k = 1;
  Dxx = 1/(h*h) * ( DOG(x+1,y) - 2*DOG(x,y) + DOG(x-1,y) );
  Dyy = 1/(k*k) * ( DOG(x,y+1) - 2*DOG(x,y) + DOG(x,y-1) );

  # and the mixed partial derivative is:
  Dxy = 1/(4*h*k) * ( DOG(x+1,y+1) - DOG(x+1,y-1) - DOG(x-1,y+1) + DOG(x-1,y-1) );
  
  TrH = Dxx + Dyy;
  DetH = Dxx*Dyy - Dxy*Dxy;
  
  r = 10;   # value used in Lowe's paper
  
  if(DetH!=0)
    if (TrH/DetH < (r+1)*(r+1)/r)
      D_new = [D_new D(:,i)];  
    endif
  endif
  ...
  gauss_filter = fspecial('gaussian', kernelSize, sig);
  #imblur = imfilter(I, gauss_filter);
  [Ix, Iy] = gradient(I);
...
#hold on, plot(D_harris(1,:)', D_harris(2,:)', 'r'); drawnow;
...
figure, imagesc(images{1}), hold off, axis image, colormap gray
hold on, scatter(D_harris(2,:)',D_harris(1,:)',D_harris(5,:)','y'); drawnow;
