...
# Assign orientation to the keypoints
D_new = [];
for p=1:size(D_harris)(2)
    ...
    for k=-hw:hw
        for l=-hw:hw
            ...
            m = sqrt( (I(x,y-1)-I(x,y+1))^2 + (I(x-1, y)-I(x+1, y))^2 ); % calc m
            theta = atan( (I(x+1, y)-I(x-1, y))/(I(x, y+1)-I(x, y-1)) );  % calc theta
            if theta<0
                theta = 2*pi+theta;
            end
            theta = theta*180/pi;
            
            idx = idivide(theta, 10);   % find the corresponding bin of the histogram
            hist(idx+1) = hist(idx+1) + m * gauss(k+hw+1, l+hw+1);  # weight and add to histogram
        end
    end
    %     hist
